 ...
 * 
 */
package com.wftp.login.controller;
...
import com.wftp.login.client.FlightBookClientProxy;
import com.wftp.login.client.FlightSearchClientProxy;
import com.wftp.login.client.RegistrationUserClientProxy;
import com.wftp.login.exception.FlightException;
import com.wftp.login.model.FlightBookDetails;
import com.wftp.login.model.FlightSearchDetails;
import com.wftp.login.model.NotificationMail;
import com.wftp.login.model.UserRegistration;
import com.wftp.login.repository.FlightExceptionRepository;
import com.wftp.login.service.EmailService;
...
public class FlightBookDetailsController {
	...
	public String getFlightDetailsByFlightIdAndFlightName(Model model, @PathVariable(value = "id") Long flightId, 
		...
		if(null != flightBookDetails) {
			if(StringUtils.hasText(flightBookDetails.getUserName())) {
				...
				if(null != userRegistration) {
					...
					flightBookDetails = flightBookClientProxy.saveBookingDetails(flightBookDetails);
				}
			} else {
				flightException = new FlightException(flightBookDetails.getBookId(), flightBookDetails.getFlightName(), 
	...
	public String bookTicket(Model model, FlightBookDetails flightBookDetails) {
		FlightException flightException = null;
		if(null != flightBookDetails) {
			if(null != flightBookDetails.getBookId()) {
				flightBookDetails = flightBookClientProxy.getBookingFlightDetailsById(flightBookDetails.getBookId());
	...
	public String viewBookingDetails(Model model, @PathVariable(value = "id") Long bookId, 
    		@PathVariable(value = "fname") String flightName) {
	...
	public String deleteBookingDetails(Model model, @PathVariable(value = "id") Long bookId, 
    		@PathVariable(value = "fname") String flightName) {
		FlightBookDetails flightBookDetails = null;
    	FlightException flightException = null;
    	if(null != bookId) {
	...
	private FlightBookDetails convertToFlightBookDetails(FlightBookDetails flightBookDetails, 
		...
		if(null != flightSearchDetails) {
			flightBookDetails.setFlightId(flightSearchDetails.getFlightId());
	...
	private FlightBookDetails convertToFlightBookDetails(FlightBookDetails flightBookDetails, 
		...
		if(null != userRegistration) {
			flightBookDetails.setUserId(userRegistration.getUserId());
