package com.wftp.registration.controller;
...
import com.wftp.registration.exception.UserResourceException;
import com.wftp.registration.model.UserRegistration;
import com.wftp.registration.repository.UserRegistrationRepository;
import com.wftp.registration.utility.PasswordGenerator;
...
public class UserRegistrationController {
	...
	@GetMapping("/user/{id}")
    public UserRegistration getRegisterUserById(@PathVariable(value = "id") Long userId) {
    	UserRegistration userRegistration = userRegistrationRepository.findById(userId)
    			.orElseThrow(() -> new UserResourceException("UserName not found", userId.toString()));
    ...
    public void deleteRegisterUser(@PathVariable(value = "id") Long userId) {
		UserRegistration userRegistration = userRegistrationRepository.findById(userId)
	            .orElseThrow(() -> new UserResourceException("UserId not found", userId.toString()));
